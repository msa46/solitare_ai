...
class Pile:
    ...
    def selected(self, mouse_position, piles):
        ...
        selected_cards = []
        deselect_pile = False
        # print("fucking mouse position ISSSSSSS:",mouse_position)
        for index, card in enumerate(self.cards):
                card_clicked = card.check_if_clicked(mouse_position)
                # print("card_clicked is",card_clicked,index)
    ...
    def transfer_cards(self, selected_cards, pile_to_transfer_to, ranks):
        if self.valid_transfer(pile_to_transfer_to, selected_cards, ranks):
            print("selected cards is:",selected_cards)
    ...
    def check_if_clicked(self, mouse_position):
        ...
        # print("self is:", self.x,self.y)
        # print("these two fucking pieces of shit are:" ,mouse_x,self.x)
        # print("this shit is:",self.x - 1 <= mouse_x)
        if (self.x - 1 <= mouse_x < self.x + self.card_width) and (self.y - 1 <= mouse_y < self.y + self.height):
            # print("PASSSSS")
            return True 
        else:
            return False
